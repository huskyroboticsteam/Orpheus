...
#define NOW std::chrono::high_resolution_clock::now();
    ...
    int new_width = img_cv.cols / 2;
    int new_height = img_cv.rows / 2;
    ...
    {
        if(obstacle[i])
            result.obstacles.push_back(rects[i]);
    }
    return result;
}
...
int gsInit(const char* camera_path)
{
    ...
    // thread to get images and put into stream
    t2 = std::thread(get_images);
...
int _main(int argc, char *argv[])
{
    ...
    for(char key = ' '; key != 'q'; key = cv::waitKey(10))
    {
        ...
        {
            ObstacleDetection d = get_obstacle_data();
            auto obstacles = std::move(d.obstacles);
            //end = NOW;
            //ms = TIME;
