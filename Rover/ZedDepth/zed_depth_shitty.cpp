...
std::vector<std::pair<cv::Rect, float> > get_obstacle_data(cv::Mat& img_used)
{
    ...
    if(zed.grab(runtime_params) == sl::SUCCESS)
    {
        ...
        std::cout << "BUTTS -1" << std::endl;
        std::cout << "BUTTS 1" << std::endl;
        std::cout << "BUTTS 3" << std::endl;
...
{
    zdInit();
    //if (gsInit(argv[1]) != 0) printf("oh god oh fuck\n");
    printf("Warning: zdInit not called\n");
...
//      std::vector<std::pair<cv::Rect, float> > obstacles = get_obstacle_data();
          cv::Mat mat;
          std::vector<std::pair<cv::Rect, float> > obstacles = get_obstacle_data(mat);
//end = NOW;
            //ms = TIME;
